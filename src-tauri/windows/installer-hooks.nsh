...
; (claude / codex / opencode / ...), and codeg-browser-sidecar.exe owns the
; managed Browser runtime. Windows does not propagate parent death to
; descendants the way Unix does, so stale companions from a previous session
; can keep either binary locked. The installer then fails to overwrite it with:
;
...
!macro NSIS_HOOK_PREINSTALL
  DetailPrint "Stopping any running Codeg companion processes..."
  nsExec::Exec 'taskkill /F /T /IM codeg-mcp.exe'
  Pop $0
  nsExec::Exec 'taskkill /F /T /IM codeg-browser-sidecar.exe'
  Pop $0
  ; Small grace period so the OS releases file handles before the
  ; installer attempts to overwrite either companion binary.
  Sleep 500
!macroend
